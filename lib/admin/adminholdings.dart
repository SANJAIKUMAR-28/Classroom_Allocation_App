...
class adminholdings extends StatefulWidget {
  final String clsname;
  final String frmdate;
  final String todate;
  const adminholdings({Key? key, required this.clsname, required this.frmdate, required this.todate}) : super(key: key);

  @override
  State<adminholdings> createState() => _adminholdingsState();
}

class _adminholdingsState extends State<adminholdings> {
  Query dbRef = FirebaseDatabase.instance.ref().child('REQUEST');
  Widget listItem({required Map request}) {
    return SingleChildScrollView(
      child:((widget.clsname==request['CLASS NAME'])&& request['VALUE']=='2'&& ((date(request['FROM DATE'])==date(widget.frmdate)&&day(request['FROM DATE'])>=day(widget.frmdate))&&(date(request['TO DATE'])==date(widget.todate)&&day(request['TO DATE'])<=day(widget.todate))))?Container(
