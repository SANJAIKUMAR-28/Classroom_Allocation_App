...
import 'package:newlogin/admin/classdetails.dart';
class adminib extends StatefulWidget {
  const adminib({Key? key}) : super(key: key);

  @override
  State<adminib> createState() => _adminibState();
}

class _adminibState extends State<adminib> {
  @override
  ...
  DatabaseReference reference = FirebaseDatabase.instance.ref().child('IBCLASS');
        ...
        child: Column(
          ...
          children: [
            if(request['CLASS-TYPE']=='Classroom'&&((int.parse(request['STRENGTH'])>=strength)&& clsvalue==true )&&!(request['SPEAKER-PROJECTOR']=='No'&&spvalue==true)&&!(request['COMPUTER']=='No'&&comvalue==true))
              ibclass(request['CLASS-NAME'], request['STRENGTH'], request['SPEAKER-PROJECTOR'], request['COMPUTER'],request['key'],request['FLOOR']),
  ...
  Material ibclass(String classnum,String strength,String speakerAndprojector,String ac,String key,String floor) {
    return Material(
      elevation: 4,
      color: Colors.white,
