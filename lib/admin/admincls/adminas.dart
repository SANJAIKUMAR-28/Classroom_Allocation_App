...
class adminas extends StatefulWidget {
  const adminas({Key? key}) : super(key: key);

  @override
  State<adminas> createState() => _adminasState();
}

class _adminasState extends State<adminas> {
  @override
  ...
  DatabaseReference reference = FirebaseDatabase.instance.ref().child('ASCLASS');
        ...
        child: Column(
          ...
          children: [
            if(request['CLASS-TYPE']=='Classroom'&&((int.parse(request['STRENGTH'])>=strength)&& clsvalue==true )&&!(request['SPEAKER-PROJECTOR']=='No'&&spvalue==true)&&!(request['COMPUTER']=='No'&&comvalue==true))
              asclass(request['CLASS-NAME'], request['STRENGTH'], request['SPEAKER-PROJECTOR'], request['COMPUTER'],request['key'],request['FLOOR']),
  ...
  Material asclass(String classnum,String strength,String speakerAndprojector,String ac,String key,String floor) {
    return Material(
      elevation: 4,
      color: Colors.white,
