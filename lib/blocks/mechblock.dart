...
class _mechBlockState extends State<mechBlock> {
        ...
        child: Column(
          ...
          children: [
            if(request['CLASS-TYPE']=='Classroom'&&((int.parse(request['STRENGTH'])>=strength)&& clsvalue==true )&&!(request['SPEAKER-PROJECTOR']=='No'&&spvalue==true)&&!(request['COMPUTER']=='No'&&comvalue==true))
              mechclass(request['CLASS-NAME'], request['STRENGTH'], request['SPEAKER-PROJECTOR'], request['COMPUTER'],request['FLOOR']),
  ...
  Material mechclass(String classnum,String strength,String speakerAndprojector,String ac,String floor) {
    return Material(
      elevation: 4,
      color: Colors.white,
