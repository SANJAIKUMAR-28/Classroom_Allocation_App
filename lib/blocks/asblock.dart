...
class _asBlockState extends State<asBlock> {
        ...
        child: Column(
          ...
          children: [
            if(request['CLASS-TYPE']=='Classroom'&&((int.parse(request['STRENGTH'])>=strength)&& clsvalue==true )&&!(request['SPEAKER-PROJECTOR']=='No'&&spvalue==true)&&!(request['COMPUTER']=='No'&&comvalue==true))
              asclass(request['CLASS-NAME'], request['STRENGTH'], request['SPEAKER-PROJECTOR'], request['COMPUTER'],request['FLOOR']),
  ...
  Material asclass(String classnum,String strength,String speakerAndprojector,String ac,String floor) {
    return Material(
      elevation: 4,
      color: Colors.white,
